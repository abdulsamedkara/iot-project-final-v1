# c9a327cc-cea5-48db-9a9b-cfd67f8c919a implementation handoff

This archive is the source of truth for turning the design into production code. Start from `smartlab-asistan.html`, then preserve the visual system, responsive behavior, and interactions found in the exported files.
...
## Source map
- Primary entry: `smartlab-asistan.html`
...
- A single self-contained `smartlab-asistan.html` is acceptable only when the export truly contains one user-facing screen and its CSS/JS are structured enough to extract tokens, components, states, and behavior.
...
1. Open `smartlab-asistan.html` and `DESIGN-MANIFEST.json`; identify every screen file, launcher/overview file, app module, and interaction before coding.
...
1. Inspect `smartlab-asistan.html` and `DESIGN-MANIFEST.json` first and identify reusable components before coding.
