...
esp_err_t dht11_read(gpio_num_t pin, dht11_reading_t *data) {
    ...
    if (wait_for_state(pin, 0, 80) == -1) { xTaskResumeAll(); return ESP_ERR_TIMEOUT; }
    if (wait_for_state(pin, 1, 80) == -1) { xTaskResumeAll(); return ESP_ERR_TIMEOUT; }
    if (wait_for_state(pin, 0, 80) == -1) { xTaskResumeAll(); return ESP_ERR_TIMEOUT; }

    for (int i = 0; i < 40; i++) {
        if (wait_for_state(pin, 1, 80) == -1) { xTaskResumeAll(); return ESP_ERR_TIMEOUT; }
        
        int high_time = wait_for_state(pin, 0, TIMEOUT_US);
        if (high_time == -1) { xTaskResumeAll(); return ESP_ERR_TIMEOUT; }
